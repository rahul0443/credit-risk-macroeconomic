...
Stage 3 of 6 — LightGBM Hyperparameter Search
...
Pipeline order:
  ...
  3_model_lgbm_search.py           ← THIS FILE (Colab only, one-time)
  4_model_v3.py                    ← run locally (uses tuned params from 2 & 3)
  5_run_pipeline.py                ← orchestrates 1 + 4 end-to-end

Builds on model_v2.py (LightGBM + addr_state target encoding, AUC=0.7091).
Goal: find better LGBM hyperparameters to push toward 0.72+.

Setup in Colab:
    1. Upload accepted_2007_to_2018Q4.csv, FEDFUNDS.csv, UNRATE.csv
       to your Google Drive under a folder named 'capstone'
    2. Run this cell first to mount Drive and install dependencies:

        from google.colab import drive
        drive.mount('/content/drive')
        !pip install lightgbm scikit-learn pandas numpy matplotlib -q

    3. Update the three FILE paths below to match your Drive folder
    4. Run the script — expect 20-40 min on Colab free tier
       (LightGBM is faster than XGBoost, so shorter than the XGB search)

What this does:
- Loads the full LendingClub dataset (same pipeline as model_v2.py)
- Applies addr_state target encoding (same as model_v2.py)
- Runs RandomizedSearchCV (40 combinations, 3-fold CV) on LightGBM
- Trains best model with early stopping on val set for final eval
- Saves best params + full results to JSON
- Reports best test ROC-AUC vs model_v2.py baseline (0.7091)

Progress tracker:
  model_improvement.py  — XGB: 0.702,  LR: 0.693
  model_v2.py           — LGBM: 0.7091 (default params), XGB: 0.7048
  model_lgbm_search.py  — LGBM tuned (this file — target: 0.71+)
"""
...
OUTDIR = "/content/drive/MyDrive/capstone/lgbm_search_outputs"
...
N_ITER      = 40       # random combinations to try (LightGBM is fast enough for 40)
...
# Search space covers the primary LightGBM complexity and regularisation axes:
#   num_leaves       — tree expressiveness; more leaves = more complex model
#   learning_rate    — shrinkage per tree; lower = better generalisation but slower
#   min_child_samples — minimum data in a leaf; higher = more regularisation
#   subsample        — row sampling per tree to reduce overfitting
#   colsample_bytree — feature sampling per tree to reduce overfitting
#   reg_lambda       — L2 regularisation on leaf weights
#   reg_alpha        — L1 regularisation (promotes sparsity)
#   n_estimators     — capped low for CV speed; final model uses 1000 + early stopping
LGBM_PARAM_DIST = {
...
# Default params from model_v2.py — used as the baseline to beat.
LGBM_BASELINE_PARAMS = {
    ...
    "reg_lambda":        1.0,
    "reg_alpha":         0.0,
    "n_estimators":      1000,  # model_v2.py used early stopping; best iteration was ~240
}
...
def drop_all_missing_cols(
    ...
    if all_missing:
        print(f"[{label}] Dropping {len(all_missing)} all-missing cols")
...
def clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Must be called on all splits before fitting ColumnTransformer, because
    the transformer operates on the original DataFrame in-place.
    Percentage strings (e.g. '13.5%') are converted to numeric here
...
def target_encode_state(
    ...
    train_col   = X_train[col].astype(str).str.strip().str.upper()
    temp        = pd.DataFrame({"state": train_col, "y": y_train})
    ...
    print(f"  Target encoded '{col}' -> '{new_col}' "
          f"({len(encode_map)} states, global_mean={global_mean:.4f})")
    return X_train, X_val, X_test
...
def build_preprocessor(X: pd.DataFrame) -> Tuple[ColumnTransformer, List[str], List[str]]:
    ...
    Percentage strings are converted inside this function before type detection.
...
def plot_roc(y_true: np.ndarray,
    ...
    plt.title("ROC Curve — Baseline vs Tuned LightGBM (Test)")
...
def main() -> None:
    """Run the LightGBM hyperparameter search end-to-end.
    ...
    Steps:
      ...
      7.  Target-encode addr_state on the training set.
      8.  Clean DataFrames and fit the preprocessor.
      9.  Fit and evaluate the baseline LightGBM model.
      10. Run RandomizedSearchCV over LGBM_PARAM_DIST.
      11. Re-train best params with early stopping on the val set.
      12. Evaluate the best model on the held-out test set.
      13. Save results JSON and top-10 combinations CSV.
      14. Print copy-paste snippet for embedding params in 4_model_v3.py.
    """
    ...
    try:
        ...
        raise ImportError(
            ...
            "Run in Colab:  !pip install lightgbm -q"
        )
    ...
    print(f"After target filter: {loans.shape[0]:,} rows")
    ...
    print("=== Load macro (excluded from features) ===")
    fed   = read_fred_csv(FEDFUNDS_FILE, "FEDFUNDS")
    unr   = read_fred_csv(UNRATE_FILE,   "UNRATE")
    macro = engineer_macro(fed, unr)
    loans = loans.merge(macro, left_on="issue_month", right_on="month",
                        how="left").drop(columns=["month"])
    ...
    if "earliest_cr_line" in loans.columns:
        ...
        loans["credit_age_months"] = (
            (loans["issue_month"] - ecl).dt.days / 30.44
        ).clip(lower=0)
    ...
    X = loans.drop(
        columns=[c for c in non_feature if c in loans.columns] + macro_cols,
        errors="ignore",
    )
    print(f"Borrower-only feature count: {X.shape[1]}")
    ...
    Xb_train = train_df[X.columns].copy()
    Xb_val   = val_df[X.columns].copy()
    Xb_test  = test_df[X.columns].copy()

    Xb_train, Xb_val, Xb_test = drop_all_missing_cols(
        Xb_train, Xb_val, Xb_test, label="Borrower-only"
    )

    # ── 7. Target encode addr_state ───────────────────────────────────────────
    print("=== Target encode addr_state ===")
    Xb_train, Xb_val, Xb_test = target_encode_state(
        Xb_train, y_train, Xb_val, Xb_test, col="addr_state"
    )
    ...
    # ── 9. Baseline LightGBM ──────────────────────────────────────────────────
    print("\n=== Baseline LightGBM (replicating model_v2.py) ===")
    ...
    baseline_prob    = baseline_model.predict_proba(Xte)[:, 1]
    baseline_metrics = eval_metrics(y_test, baseline_prob)
    print(f"Baseline Test ROC-AUC: {baseline_metrics['roc_auc']:.4f}  "
          f"(model_v2.py reported: 0.7091)")

    # ── 10. Randomized search ─────────────────────────────────────────────────
    # Note: RandomizedSearchCV calls .fit() without eval_set, so early stopping
    # cannot run inside CV folds. A fixed n_estimators is used during search;
    # the best config is then re-trained with early stopping in step 11.
    print(f"\n=== Randomized Search ({N_ITER} combinations, {CV_FOLDS}-fold CV) ===")
    print("LightGBM is fast — expect ~20-40 min on Colab free tier.")
    print("Progress (verbose=2 shows each combination):\n")
    ...
    search = RandomizedSearchCV(
        ...
        random_state=RANDOM_SEED,
        n_jobs=1,     # LightGBM parallelises internally; outer parallelism would conflict
        refit=True,
    )
    ...
    # ── 11. Re-train best params with early stopping ──────────────────────────
    # CV used a fixed n_estimators; here we use n_estimators=1000 with early
    # stopping to let the model find the optimal iteration count on val data.
    print("\n=== Re-train best params with early stopping on val set ===")
    best_p = search.best_params_.copy()
    best_p_final = {k: v for k, v in best_p.items() if k != "n_estimators"}
    best_p_final["n_estimators"] = 1000
    ...
    tuned_prob    = tuned_model.predict_proba(Xte)[:, 1]
    tuned_metrics = eval_metrics(y_test, tuned_prob)

    improvement = tuned_metrics["roc_auc"] - baseline_metrics["roc_auc"]
    v2_baseline = 0.7091

    print(f"\n{'='*55}")
    print(f"=== FINAL RESULTS (Test ROC-AUC) ===")
    print(f"{'='*55}")
    print(f"model_v2.py baseline  : {v2_baseline:.4f}")
    print(f"Baseline (replicated) : {baseline_metrics['roc_auc']:.4f}")
    print(f"Tuned LightGBM        : {tuned_metrics['roc_auc']:.4f}  "
    ...
    # Feature importance from the sparse matrix uses index-based names.
    try:
        n_features = tuned_model.n_features_in_
        fi = tuned_model.feature_importances_
        top_idx = np.argsort(fi)[::-1][:20]
        print(f"\nTop 20 features by importance (gain):")
        for rank, idx in enumerate(top_idx, 1):
            print(f"  {rank:2d}. Feature_{idx:03d}: {fi[idx]:.1f}")
    except Exception:
        pass  # feature names not available from sparse input — that's fine
    ...
    results = {
        ...
        "tuned_lgbm": {
            ...
            "final_params_with_es":  best_p_final,
    ...
    results_path = os.path.join(OUTDIR, "lgbm_search_results.json")
    ...
    csv_path = os.path.join(OUTDIR, "top10_lgbm_params.csv")
    cv_df.to_csv(csv_path, index=False)
    print(f"Top-10 combinations saved to: {csv_path}")

    plot_roc(
        y_test,
        curves={
            f"Baseline LGBM ({baseline_metrics['roc_auc']:.4f})": baseline_prob,
            f"Tuned LGBM    ({tuned_metrics['roc_auc']:.4f})":    tuned_prob,
        },
        out_path=os.path.join(FIGDIR, "roc_lgbm_baseline_vs_tuned.png"),
    )

    # ── 14. Copy-paste snippet ─────────────────────────────────────────────────
    # Print the best params in a format ready to paste into 4_model_v3.py.
    print("\n" + "="*55)
    print("=== COPY-PASTE THIS INTO model_v2.py LGBM_PARAMS ===")
    print("="*55)
    paste_params = {k: v for k, v in best_p_final.items()}
    print("LGBM_PARAMS = {")
    for k, v in paste_params.items():
